...
df[' DTHR LIMITE'] = pd.to_datetime(df[' DTHR LIMITE'])
...
atendimento_fora = df.loc[(df[' STATUS'] == "EM ATENDIMENTO")& (df['MOBILE'] == "NÃO")& (df['PRÉ-BAIXA'] == "NÃO")& (df['PB CENTRAL'] == "NÃO")]
...
def select_atraso():
   ...
   vencimento2 = tec_atraso.set_index([' DTHR LIMITE'])
   ...
   end = tec_atraso.set_index([' ENDEREÇO'])
   texto_tec = "                  Técnico: "
   nome_cliente = tec_atraso.set_index([' NOME FANTASIA'])
   qtd_age = tec_atraso.set_index([' QT. AGENDAMENTO'])
   mot_age = tec_atraso.set_index([' MOTIVO AGENDAMENTO'])
   contact = tec_atraso.set_index([' CONTATO NO E.C.'])
   cli_cel = tec_atraso.set_index([' CELULAR'])
   cli_tel = tec_atraso.set_index([' TELEFONE'])
   date = []

   
   for x in range(len(vencimento)):
      if vencimento[x] < local_dt:
         date.append(vencimento[x])         
   ...
   my_text.insert(END,len(date))
   my_text.insert(END,"                          Total: ")
   my_text.insert(END,len(os_tec))
   my_text.insert(END,"\n")
   my_text.insert(END,quebra)
   for x in range(len(os_tec)):
      ...
      my_text.insert(END,vencimento[x].strftime("%d/%m/%y - %H:%M:%S"))
      my_text.insert(END,"\n")
      ...
      my_text.insert(END,"\n")
      my_text.insert(END,'Endereço: ')
      my_text.insert(END,end.index[x])

      my_text.insert(END,"\n")
      my_text.insert(END,'Cliente: ')
      my_text.insert(END,nome_cliente.index[x])
      my_text.insert(END,"\n")
      my_text.insert(END,'Contato: ')
      my_text.insert(END,contact.index[x])
      my_text.insert(END,"\n")
      my_text.insert(END,'Celular: ')
      my_text.insert(END,cli_cel.index[x])
      my_text.insert(END,"\n")
      my_text.insert(END,'Telefone: ')
      my_text.insert(END,cli_tel.index[x])
      my_text.insert(END,"\n")
      my_text.insert(END,"\n")
      my_text.insert(END,'Agendamento: ')
      my_text.insert(END,int(qtd_age.index[x]))
      my_text.insert(END,"\n")
      my_text.insert(END,'Motivo: ')
      my_text.insert(END,mot_age.index[x])
      my_text.insert(END,"\n")
   
      
      my_text.insert(END,"\n")
      if(vencimento[x] <= local_dt):
         my_text.insert(END,'Justificativa Base: ')
         my_text.insert(END,"\n")
         my_text.insert(END,'Previsão: ')
         my_text.insert(END,"\n")
      my_text.insert(END,("*********")*15)
      my_text.insert(END,"\n")
...
listbox_qtd_tot_atend = Listbox(root,bg=fundo_total_cores, fg="black")
listbox_qtd_tot_atend.place(relx=0.78,rely=0.735,relheight=.03,relwidth=.021)

listbox_qtd_tot_atraso = Listbox(root,bg=fundo_total_cores, fg="black")
listbox_qtd_tot_atraso.place(relx=0.81,rely=0.735,relheight=.03,relwidth=.02)
...
listbox_serv_atraso = Listbox(root,bg=list_color, fg="red")
...
listbox_qtd_atraso = Listbox(root,bg=list_color, fg="blue")
listbox_qtd_atraso.place(relx=0.78,rely=0.43,relheight=.3,relwidth=.023)

listbox_qtd_total_atend = Listbox(root,bg='blue', fg="black")
listbox_qtd_total_atend.place(relx=0.81,rely=0.43,relheight=.3,relwidth=.021)
...
listbox_qtd_tot_atend.insert(END,sum(atendimento_fora_tec))
listbox_tot_atraso.insert(END,len(atendimento_fora2))
listbox_tot_finaliz.insert(END,len(total_cancel)+len(total_enc))

vencida = atendimento_fora.set_index([' DTHR LIMITE'])
date = []
#for x in range(len(atendimento_fora)):#
   #if atendimento_fora[x] < local_dt:
    #  date.append(atendimento_fora[x])    
for x in range(len(vencida)):
   if vencida.index[x] < local_dt:
      #listbox_qtd_total_atend.insert(END,vencida.index[x])
      date.append(vencida.index[x])
print(date)
listbox_qtd_tot_atraso.insert(END,len(date))
root.mainloop()